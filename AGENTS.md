...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; kind=secondmate also records home= and projects=; cursor hook creation records cursor_hook_created=1 for safe teardown cleanup; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, appends pr= and GitHub's pr_head= when available; fm-x-link appends x_request=, x_request_ts=, and x_followups= for an X-mention-originated task (section 14)
...
A backend spawn refusal - a missing dependency, an unauthenticated socket, a version gate, unsupported mode, or unsupported harness/backend pairing - must be surfaced to the captain as a blocker; never silently retry the spawn on a different backend to work around it.
...
For cursor, the turn-end hook is a git-excluded per-worktree `.cursor/hooks.json` stop hook, merged only when safe; teardown removes firstmate's hook entry and uses `cursor_hook_created=1` to remove a hook file firstmate created from scratch.
