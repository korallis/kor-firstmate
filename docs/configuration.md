...
The `cursor` harness is spawn-verified only on `tmux` and `herdr`; `fm-spawn.sh` still refuses cursor on `zellij`, `orca`, and `cmux` until those backends have cursor-specific submit verification.
...
The verified adapter knowledge - busy signatures, interrupt and exit commands, skill-invocation syntax, per-harness model/effort handling, and other quirks - lives in [`.agents/skills/harness-adapters/SKILL.md`](../.agents/skills/harness-adapters/SKILL.md).
...
For cursor, `fm-spawn.sh` installs a git-excluded per-worktree `.cursor/hooks.json` stop hook on the tmux and herdr backends, records `cursor_hook_created=1` when firstmate created that hook file from scratch, and `fm-teardown.sh` removes only firstmate's hook entry.
...
FM_BUSY_REGEX='esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel|Add a follow-up.*ctrl\+c to stop'   # busy-pane signatures, shared by watcher, fm-crew-state pane fallback, and tmux helper
FM_COMPOSER_IDLE_RE=    # optional empty-composer placeholder regex, applied after dim-ghost and border stripping
...
FM_SUPERVISOR_HARNESS=             # optional supervisor pane harness override; cursor supervisors are still tmux-only and use this to read the arrow-row composer instead of tmux cursor_y
